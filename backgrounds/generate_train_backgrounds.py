    ...
    with torch.no_grad():
        while len(saved_seqs) < TARGET_COUNT and len(seen_indices) < len(df):
            ...
            if len(saved_seqs) == chunk_size or (len(saved_seqs) + chunk_idx * chunk_size) == TARGET_COUNT:
                output_path = f"/scratch1/smaruj/background_generation/train_backgrounds/background_sequences_chunk_{chunk_idx}.fasta"
                with open(output_path, "w") as f:
                    for i, (chrom, start, end, seq) in enumerate(saved_seqs):
                        f.write(f">shuffled_{chunk_idx*chunk_size + i}_chr{chrom}_{start}_{end}_scd{scd_scores[i]:.2f}_totvar={totvar_scores[i]:.2f}\n")
                        f.write(f"{seq}\n")
