...
def main():
    ...
    model0 = SeqNN()
    model0.load_state_dict(torch.load("/scratch1/smaruj/Akita_pytorch_models/finetuned/human_models/Rao2014_IMR90/models/Akita_v2_human_Rao2014_IMR90_model0_finetuned.pth", map_location=device))
    model0.eval()
    
    model1 = SeqNN()
    model1.load_state_dict(torch.load("/scratch1/smaruj/Akita_pytorch_models/finetuned/human_models/Rao2014_IMR90/models/Akita_v2_human_Rao2014_IMR90_model1_finetuned.pth", map_location=device))
    model1.eval()
    
    model2 = SeqNN()
    model2.load_state_dict(torch.load("/scratch1/smaruj/Akita_pytorch_models/finetuned/human_models/Rao2014_GM12878/models/Akita_v2_human_Rao2014_GM12878_model0_finetuned.pth", map_location=device))
    model2.eval()

    model3 = SeqNN()
    model3.load_state_dict(torch.load("/scratch1/smaruj/Akita_pytorch_models/finetuned/human_models/Rao2014_GM12878/models/Akita_v2_human_Rao2014_GM12878_model1_finetuned.pth", map_location=device))
    model3.eval()
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        target_m0 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_IMR90_strong_boundary/model0/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        target_m1 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_IMR90_strong_boundary/model1/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        target_m2 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_GM12878_weak_boundary/model0/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        target_m3 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_GM12878_weak_boundary/model1/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
    
        y_bar_list = [target_m0, target_m1, target_m2, target_m3]
        
        tower_output_path_list = [f"/scratch1/smaruj/generate_cell_type_specific_features/tower_output_IMR90/model0/{chrom}_{pred_start}_{pred_end}_tower_out.pt",
                                f"/scratch1/smaruj/generate_cell_type_specific_features/tower_output_IMR90/model1/{chrom}_{pred_start}_{pred_end}_tower_out.pt",
                                f"/scratch1/smaruj/generate_cell_type_specific_features/tower_output_GM12878/model0/{chrom}_{pred_start}_{pred_end}_tower_out.pt",
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.pt_files_dir}/IMR90_strong_GM12878_weak_results/{chrom}_{pred_start}_{pred_end}_slice.pt")
        
    df.to_csv(f"{args.pt_files_dir}/fold{FOLD}_{target_c}_IMR90_strong_GM12878_weak_results.tsv", sep="\t", index=False)
