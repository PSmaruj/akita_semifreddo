...
def main():
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        target_m2 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_HCT116_weak_boundary/model0/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        target_m3 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_HCT116_weak_boundary/model1/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.pt_files_dir}/IMR90_strong_HTC116_weak_results/{chrom}_{pred_start}_{pred_end}_slice.pt")
        
    df.to_csv(f"{args.pt_files_dir}/fold{FOLD}_{target_c}_IMR90_strong_HTC116_weak_results.tsv", sep="\t", index=False)
