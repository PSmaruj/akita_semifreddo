...
def main():
    ...
    for i, row in enumerate(df[0:3].itertuples(index=False)):
        ...
        target_m2 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_ncx_NPC_no_boundary/model0/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        target_m3 = torch.load(f"/scratch1/smaruj/generate_cell_type_specific_features/target_ncx_NPC_no_boundary/model1/{chrom}_{pred_start}_{pred_end}_target.pt", weights_only=True, map_location=device)
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.pt_files_dir}/mESC_weak_ncx_NPC_strong_results_max_NB/{chrom}_{pred_start}_{pred_end}_slice.pt")
