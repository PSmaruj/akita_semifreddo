...
def main():
    ...
    for i, row in enumerate(df[0:3].itertuples(index=False)):
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.pt_files_dir}/IMR90_weak_HTC116_strong_results/{chrom}_{pred_start}_{pred_end}_slice.pt")
        
    df.to_csv(f"{args.pt_files_dir}/fold{FOLD}_{target_c}_IMR90_weak_HTC116_strong_results.tsv", sep="\t", index=False)
