...
def main():
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.output_dir}/tau_{args.tau}/{chrom}_{pred_start}_{pred_end}_slice.pt")
    ...
    df.to_csv(f"{args.output_dir}/tau_{args.tau}_df.tsv", sep="\t", index=False)
