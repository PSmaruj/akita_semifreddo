"""
run_dot_alphagenome.py
----------------------
Run Alpha Genome predictions on original and dot-designed sequences
for all 8 folds and save per-fold TSVs with dot scores.
"""

import pandas as pd
from alphagenome.models import dna_client
from helper import dot_score, score_fasta_dir
...
for fold in FOLDS:
    ...
    df["alpha_og_score"] = score_fasta_dir(dna_model, df, og_fasta_dir,  dot_score, label="original")

    print(f"\nDesigned sequences ({len(df)} loci):")
    df["alpha_ed_score"] = score_fasta_dir(dna_model, df, mod_fasta_dir, dot_score, label="designed")
