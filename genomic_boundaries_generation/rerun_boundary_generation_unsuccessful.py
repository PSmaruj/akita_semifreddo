...
def main():
    ...
    df = pd.read_csv(f"/scratch1/smaruj/generate_genomic_boundary/rerun_unsuccessful/unsuccessful_all_folds_-0.5.tsv", sep="\t")
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        torch.save(x_bar_slice_0[:,:,padding:-padding], f"{args.pt_files_dir}/rerun_unsuccessful/fold{FOLD}/{chrom}_{pred_start}_{pred_end}_slice.pt")
        
    df.to_csv(f"{args.pt_files_dir}/rerun_unsuccessful/rerun_unsuccessful_{target_c}_genomic_windows_table_steps.tsv", sep="\t", index=False)
