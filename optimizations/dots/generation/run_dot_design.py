...
Batch dot optimisation over all genomic windows in a fold's flat-regions table.
...
from utils.optimization_loop_utils import run_one_design_dot, run_fold
...
DEFAULT_TARGET_BASE_DIR   = f"{_PROJ}/optimizations/dots/targets"
DEFAULT_RESULTS_BASE_DIR  = f"{_PROJ}/optimizations/dots"
DEFAULT_FLAT_REGIONS_BASE = f"{_PROJ}/analysis/flat_regions/mouse_flat_regions_chrom_states_tsv"
DEFAULT_MASK_BASE_DIR     = f"{_PROJ}/optimizations/feature_masks"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Batch dot optimisation over one or more folds",
    ...
    p.add_argument("--mask_base_dir",     default=DEFAULT_MASK_BASE_DIR)
...
def main() -> None:
    ...
    # Derive anchor bin positions from inter-anchor distance
    anchor_offset = d // 2
    bin_anchor_lo = CENTER_BIN_MAP - anchor_offset
    bin_anchor_hi = CENTER_BIN_MAP + anchor_offset

    log.info(f"Device: {device}  |  Folds: {args.folds}  |  Run: {args.run_name}")
    log.info(f"Inter-anchor distance: {d}  →  bins {bin_anchor_lo} / {bin_anchor_hi}")
    log.info(f"Ledidi params — L={args.L}  tau={args.tau}  eps={args.eps}")

    # ── Shared resources (loaded once across all folds) ───────────────────────
    model       = load_model(args.model_path, device)
    mask        = torch.load(os.path.join(args.mask_base_dir, f"dot_d{d}_mask.pt"), weights_only=True).to(device)
    output_loss = LocalL1Loss(mask, n_triu=N_TRIU, reduction="sum").to(device)

    # ── Dot-specific run_one closure ──────────────────────────────────────────
    def run_one_fn(row, fold, args, out_dir):
        ...
        target = torch.load(f"{args.target_base_dir}/dot_d{d}/fold{fold}/{stem}_target.pt", weights_only=True).to(device)
        sf_wrapper = TwoAnchorSemifreddoLedidiWrapper(
            model=model, precomputed_full_output=tower, full_X=X,
            bin_lo=bin_anchor_lo, bin_hi=bin_anchor_hi,
            context_bins=CONTEXT_BINS, cropping_applied=CROPPING_APPLIED,
        )
        return run_one_design_dot(row, fold, args, sf_wrapper, output_loss, X, target, device, out_dir, BIN_SIZE)
