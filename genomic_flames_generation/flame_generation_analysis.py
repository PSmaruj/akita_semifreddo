...
class FlameGenerationDataset(Dataset):
...
def main():
    ...
    df_path = f"/scratch1/smaruj/generate_genomic_flame/results/target_{TARGET_C}/fold{FOLD}_{TARGET_C}_genomic_windows_table_steps.tsv"
    ...
    edited_dataset = FlameGenerationDataset(
        df,
        f"/scratch1/smaruj/generate_genomic_boundary/ohe_X/fold{FOLD}/",
        f"/scratch1/smaruj/generate_genomic_flame/results/target_{TARGET_C}/fold{FOLD}/"
    )
    target_dataset = TriuMatrixDataset(
        df,
        f"/scratch1/smaruj/generate_genomic_flame/targets/target_{TARGET_C}/fold{FOLD}/"
    )
    ...
    scd_values = []
    flame_mean_values = []
    og_flame_mean_values = []
    target_flame_mean_values = []
    ...
    with torch.no_grad():
        for orig_batch, edited_batch, target_batch in zip(orig_loader, edited_loader, target_loader):
            ...
            flame_mean = np.nanmean(edited_maps[:, :half_r, half_c - half_flame_width:half_c + half_flame_width], axis=(1, 2))
            flame_mean_values.extend(flame_mean)
            
            og_flame_mean = np.nanmean(orig_maps[:, :half_r, half_c - half_flame_width:half_c + half_flame_width], axis=(1, 2))
            og_flame_mean_values.extend(og_flame_mean)
            
            target_maps = from_upper_triu_batch(target_batch)
            target_flame_mean = np.nanmean(target_maps[:, :half_r, half_c - half_flame_width:half_c + half_flame_width], axis=(1, 2))
            target_flame_mean_values.extend(target_flame_mean)
    ...
    # -----------------------------
    df["SCD"] = scd_values
    df["flame_result"] = flame_mean_values
    df["flame_target"] = target_flame_mean_values
    df["flame_init"] = og_flame_mean_values
    ...
    out_path = f"/scratch1/smaruj/generate_genomic_flame/results/target_{TARGET_C}/fold{FOLD}_{TARGET_C}_genomic_windows_table_results.tsv"
